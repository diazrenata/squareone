---
title: "Appendix S2 - Full analytical methods and model results"
subtitle: "Supplemental information for Diaz and Ernest, “Maintenance of community function through compensation breaks down over time in a desert rodent community”. Now published in Ecology."
...
plot(totale_mean_gls, oera ~  resid(.))
...
dipo_glm_resid <- dipo_c_dat %>%
  mutate(resid = resid(dipo_glm))

ggplot(dipo_glm_resid, aes(resid, oera)) +
  geom_boxplot() +  geom_point(alpha = .2, color = "pink")
```
...
pb_glm_resid <- pb_glm_nointeraction$model %>%
  mutate(resid = resid(pb_glm_nointeraction))

ggplot(pb_glm_resid, aes(resid, oera)) +
  geom_boxplot() +  geom_point(alpha = .2, color = "pink")
```
