...
erod_plot <- ggplot(filter(erod_predict, !is.na(Treatment)), aes(year,erod_treatment_prop_abundance, color = Treatment, fill =  Treatment)) +
  ...
  xlab("Date") +  
  scale_x_continuous(sec.axis = dup_axis(name = NULL, breaks = unique(filter(erod_predict, !is.na(event_name))$year), labels = unique(filter(erod_predict, !is.na(event_name))$no_name)), limits = c(1988, 2020.25)) +  
...
treatl <- treatl %>%
  mutate(trt = ifelse(oplottype == "CC", "Control", "Exclosure"))

te_plot <- ggplot(treatl, aes(censusdate, total_e_ma, color = trt)) + 
  geom_line() +
  both_scale +
  era_lines +
  era_labs + theme(legend.position = c(.15, .7), legend.background = element_blank(), legend.title = element_blank()) +
  ggtitle("Total rodent energy use") +
  xlab("Date") +
  ylab("Treatment-level mean energy use (kJ / plot)")
...
gridExtra::grid.arrange(grobs = list(spei_plot, erod_plot), ncol =1)
