...
g1 <- gridExtra::grid.arrange(grobs = list(comp_plot, totale_plot), ncol = 1)
...
cplot <- ggplot(filter(both_treat_se, oplottype == "CC"), aes(censusdate, est, color = Rodents, fill = Rodents)) +
  ...
  theme(axis.title.y = element_text(size = 8), legend.title = element_blank(), legend.text = element_text(size =8))+ theme(axis.title.x = element_blank(), axis.title.y = element_blank(), title = element_text(size = 10), legend.background = element_blank(), legend.direction = "vertical")
...
```{r, fig.dim = c(6,4)}
library(grid)
g2 <- gridExtra::grid.arrange(grobs = list(cplot, eplot), ncol = 1, top = textGrob("Rodent community composition"), bottom = textGrob("Date"), left = textGrob("Proportion of total community energy use", rot = 90))

g2

```

## 2. Rodent community composition - krats
