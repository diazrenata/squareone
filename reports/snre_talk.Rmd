...
ggplot(filter(pb_glm_treat_se, oplottype %in% c("CC", "EE")), aes(censusdate, est, color = oplottype, fill = oplottype)) +
  geom_line() +
  geom_ribbon(aes(ymin = lower, ymax= upper), alpha = .3) +
...
ggplot(filter(totale_mean_pred, oplottype %in% c("CC", "EE")), aes(censusdate, emmean, color = oplottype, fill = oplottype)) +
 ...
 # era_grid +
  ...
  ylim(0, 2) +
  geom_vline(xintercept= c(as.Date("1996-01-01"), as.Date("2010-01-01"))) +
  xlab("Date")
...
e_ratios_lt <- e_ratios %>%
  filter(oplottype == "EE") %>%
  mutate(era = ifelse(era == "d_post-switch", "c_pre_switch", era))

totale_mean_gls_lt <- gls(total_e_rat ~  era,  correlation = corCAR1(form = ~ period), data = e_ratios_lt)

totale_mean_gls_lt_emmeans <- emmeans(totale_mean_gls_lt, specs = ~ era )

out <- (as.data.frame(pairs(totale_mean_gls_lt_emmeans)) %>% select(contrast, p.value) %>%
  mutate(p.value= round(p.value, 3)))
out
totale_mean_pred_lt <- as.data.frame(totale_mean_gls_lt_emmeans) %>%
  right_join(e_ratios_lt)
  
ggplot(filter(totale_mean_pred_lt, oplottype %in% c("CC", "EE")), aes(censusdate, emmean, color = oplottype, fill = oplottype)) +
  geom_line() +
  geom_ribbon(aes(ymin = lower.CL, ymax = upper.CL), alpha = .2) +
  geom_line(aes(y = total_e_rat_ma)) +
  scale_color_viridis_d(begin  = .25) +
  ...
  geom_vline(xintercept= c(as.Date("1996-01-01"), as.Date("2010-01-01"))) +
  xlab("Date") +
  geom_text(aes(x = as.Date("1998-03-01"), y = 0.05, label = "C. baileyi \narrival"), inherit.aes = F, alpha = .01)+
  geom_text(aes(x = as.Date("2012-03-01"), y = 0.05, label = "C. baileyi \n decline"), inherit.aes = F, alpha = .01)


ggplot(filter(totale_mean_pred_lt, oplottype %in% c("CC", "EE"), as.numeric(oera)  < 3), aes(censusdate, emmean, color = oplottype, fill = oplottype)) +
  geom_line() +
  geom_ribbon(aes(ymin = lower.CL, ymax = upper.CL), alpha = .2) +
  geom_line(aes(y = total_e_rat_ma)) +
  scale_color_viridis_d(begin  = .25) +
  ...
  theme(legend.position = "none") +
  ylim(0, 1.1) +
  geom_vline(xintercept= c(as.Date("1996-01-01"))) +
  xlab("Date") +
  geom_text(aes(x = as.Date("1998-03-01"), y = 0.05, label = "C. baileyi \narrival"), inherit.aes = F, alpha = .01)
out <- (totale_mean_pred_lt%>% select(oera, oplottype, emmean, lower.CL, upper.CL) %>% distinct())
out
...
```{r}
compensation_lt <- compensation %>%
  filter(oplottype == "EE") %>%
  mutate(era = ifelse(era == "d_post-switch", "c_pre_switch", era))

  

comp_mean_lt_gls <- gls(smgran_comp ~ era,  correlation = corCAR1(form = ~ period), data = compensation_lt)

comp_mean_lt_gls_emmeans <- emmeans(comp_mean_lt_gls, specs = ~ era)

out <- (as.data.frame(pairs(comp_mean_lt_gls_emmeans)) %>% mutate(p.value = round(p.value, 3)))
out
comp_mean_lt_pred <-  as.data.frame(comp_mean_lt_gls_emmeans) %>%
  right_join(compensation_lt)

ggplot(filter(comp_mean_lt_pred, oplottype %in% c("CC", "EE")), aes(censusdate, emmean, color = oplottype, fill = oplottype)) +
  geom_line() +
  geom_ribbon(aes(ymin = lower.CL, ymax = upper.CL), alpha = .2) +
 # era_grid +
  geom_line(aes(y = smgran_comp_ma)) +
  ...
  geom_vline(xintercept= c(as.Date("1996-01-01"), as.Date("2010-01-01"))) +
  xlab("Date") +
  geom_text(aes(x = as.Date("1998-03-01"), y = -0.03, label = "C. baileyi \narrival"), inherit.aes = F, alpha = .01)+
  geom_text(aes(x = as.Date("2012-03-01"), y = -0.03, label = "C. baileyi \n decline"), inherit.aes = F, alpha = .01) +
  ylab("Compensation") +
  ggtitle("Energetic compensation over time")


ggplot(filter(comp_mean_lt_pred, oplottype %in% c("CC", "EE"), as.numeric(oera) < 3), aes(censusdate, emmean, color = oplottype, fill = oplottype)) +
  geom_line() +
  geom_ribbon(aes(ymin = lower.CL, ymax = upper.CL), alpha = .2) +
 # era_grid +
  geom_line(aes(y = smgran_comp_ma)) +
  ...
  ylab("Compensation") +
  ggtitle("Energetic compensation over time")
out <- (comp_mean_lt_pred %>% select(oera, oplottype, emmean, lower.CL, upper.CL) %>% distinct())
out 
...
ctrl_dipos <- filter(treatl, oplottype == "CC") %>%
  mutate(dipo_of_total = dipo_e / total_e) %>%
  mutate(dipo_of_total_ma = maopts(dipo_of_total))

ggplot(ctrl_dipos, aes(censusdate, dipo_of_total)) +
  geom_line() +
  era_grid

ctrl_dipos_glm <- glm(dipo_of_total ~ era, data = ctrl_dipos,family = quasibinomial())
ctrl_dipos_glm_emmeans <- emmeans(ctrl_dipos_glm, specs = ~ era)
out <- (as.data.frame(pairs(ctrl_dipos_glm_emmeans)) %>% mutate(p.value = round(p.value, 3)))
out


ilink <- ctrl_dipos_glm$family$linkinv

ctrl_dipos_glm_se <- predict(ctrl_dipos_glm, type = "link", se.fit = T, newdata = ctrl_dipos) %>%
  as.data.frame() %>%
  mutate(est = ilink(fit),
         lower = ilink(fit - 2*se.fit),
         upper = ilink(fit + 2*se.fit),
         period = filter(ctrl_dipos)$period)

ctrl_dipos_glm_se <- ctrl_dipos_glm_se %>%
  right_join(select(ctrl_dipos, oera, period, censusdate, dipo_of_total_ma))


ggplot(ctrl_dipos_glm_se, aes(censusdate, est)) +
  geom_line() +
  geom_ribbon(aes(ymin = lower, ymax= upper), alpha = .3) +
  geom_line(aes(y = dipo_of_total_ma)) +
  facet_grid(cols = vars(oera), space = "free_x", scales = "free_x") +
  ylab("Dipo % of energy use") +
  ggtitle("Dipo ratio  on control plots") +
  theme(legend.position = "top")

ctrl_dipos_glm_se %>%
  select(oera, est, lower, upper) %>%
  distinct()
```
