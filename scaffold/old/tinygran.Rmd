...
```{r}
sg_e_mod <- gam(smgran_e ~ oplottype + s(period, k = 50) + s(period, by = oplottype, k = 50), family = "tw", data = treatment_means)
...
sg_rawe_ee_cc_diff <- get_exclosure_diff(sg_e_mod, sg_e_pdat,comparison_variable = "oplottype", reference_level = 1, comparison_level = 3)

sg_rawe_ee_cc_pred <- add_exclosure_diff(sg_e_pred, sg_rawe_ee_cc_diff)
plot_fitted_pred(filter(sg_rawe_ee_cc_pred, oplottype != "CE"), comparison_variable = "oplottype") +
  ggtitle("GAM diff smgran raw energy CC v EE")+
  facet_grid(cols = vars(era), scales = "free_x", space = "free") +
  theme(legend.position = "top")
...
plot_fitted_pred(filter(sg_rawe_ce_cc_pred, oplottype != "EE"), comparison_variable = "oplottype") + scale_color_viridis_d(end = .4) + scale_fill_viridis_d(end = .4) +
  ggtitle("GAM diff smgran raw energy CC v CE")+
  facet_grid(cols = vars(era), scales = "free_x", space = "free") +
  theme(legend.position = "top")


sg_rawe_ee_ce_diff <- get_exclosure_diff(sg_e_mod, sg_e_pdat,comparison_variable = "oplottype", reference_level = 2, comparison_level = 3)
sg_rawe_ee_ce_pred <- add_exclosure_diff(sg_e_pred, sg_rawe_ee_ce_diff)
plot_fitted_pred(filter(sg_rawe_ee_ce_pred, oplottype != "CC"), comparison_variable = "oplottype")+ scale_color_viridis_d(begin = .4, end = .8) + scale_fill_viridis_d(begin = .4, end = .8) +
  ggtitle("GAM diff sg raw energy CE v EE")+
  facet_grid(cols = vars(era), scales = "free_x", space = "free") +
  theme(legend.position = "top")
...
```{r}
tg_e_mod <- gam(tinygran_e ~ oplottype + s(period, k = 50) + s(period, by = oplottype, k = 50), family = "tw", data = treatment_means)
...
tg_rawe_ee_cc_diff <- get_exclosure_diff(tg_e_mod, tg_e_pdat,comparison_variable = "oplottype", reference_level = 1, comparison_level = 3)
...
tg_rawe_ce_cc_diff <- get_exclosure_diff(tg_e_mod, tg_e_pdat,comparison_variable = "oplottype", reference_level = 1, comparison_level = 2)
tg_rawe_ce_cc_pred <- add_exclosure_diff(tg_e_pred, tg_rawe_ce_cc_diff)
plot_fitted_pred(filter(tg_rawe_ce_cc_pred, oplottype != "EE"), comparison_variable = "oplottype") + scale_color_viridis_d(end = .4) + scale_fill_viridis_d(end = .4) +
  ggtitle("GAM diff tgran raw e CE v CC")+
  facet_grid(cols = vars(era), scales = "free_x", space = "free") +
  theme(legend.position = "top")


tg_rawe_ee_ce_diff <- get_exclosure_diff(tg_e_mod, tg_e_pdat,comparison_variable = "oplottype", reference_level = 2, comparison_level = 3)
tg_rawe_ee_ce_pred <- add_exclosure_diff(tg_e_pred, tg_rawe_ee_ce_diff)
plot_fitted_pred(filter(tg_rawe_ee_ce_pred, oplottype != "CC"), comparison_variable = "oplottype")+ scale_color_viridis_d(begin = .4, end = .8) + scale_fill_viridis_d(begin = .4, end = .8) +
  ggtitle("GAM diff tgran raw e CE v EE")+
  facet_grid(cols = vars(era), scales = "free_x", space = "free") +
  theme(legend.position = "top")
...
```{r}
total_e_mod <- gam(total_e ~ oplottype + s(period, k = 50) + s(period, by = oplottype, k = 50), family = "tw", data = treatment_means)
...
total_rawe_ee_cc_diff <- get_exclosure_diff(total_e_mod, total_e_pdat,comparison_variable = "oplottype", reference_level = 1, comparison_level = 3)
...
total_rawe_ce_cc_diff <- get_exclosure_diff(total_e_mod, total_e_pdat,comparison_variable = "oplottype", reference_level = 1, comparison_level = 2)
total_rawe_ce_cc_pred <- add_exclosure_diff(total_e_pred, total_rawe_ce_cc_diff)
plot_fitted_pred(filter(total_rawe_ce_cc_pred, oplottype != "EE"), comparison_variable = "oplottype") + scale_color_viridis_d(end = .4) + scale_fill_viridis_d(end = .4) +
  ggtitle("GAM diff total raw e CC v CE")+
  facet_grid(cols = vars(era), scales = "free_x", space = "free") +
  theme(legend.position = "top")


total_rawe_ee_ce_diff <- get_exclosure_diff(total_e_mod, total_e_pdat,comparison_variable = "oplottype", reference_level = 2, comparison_level = 3)
total_rawe_ee_ce_pred <- add_exclosure_diff(total_e_pred, total_rawe_ee_ce_diff)
plot_fitted_pred(filter(total_rawe_ee_ce_pred, oplottype != "CC"), comparison_variable = "oplottype")+ scale_color_viridis_d(begin = .4, end = .8) + scale_fill_viridis_d(begin = .4, end = .8) +
  ggtitle("GAM diff total raw e CE v EE")+
  facet_grid(cols = vars(era), scales = "free_x", space = "free") +
  theme(legend.position = "top")
...
ggplot(treatment_ratios, aes(censusdate, tg_of_c, color = oplottype)) +
  geom_line() +
  scale_color_viridis_d(end = .8) +
  theme(legend.position = "top")+ 
  facet_wrap(vars(era), scales= "free_x", ncol = 1)
...
Just to show that, if there was a **stronger** gain from tinygran, it would show up.

```{r}
treatment_ratios <- treatment_ratios %>%
  mutate(tg_of_c_toy = ifelse(era == "d_post-switch",
                               ifelse(oplottype != "CC",
                                      tg_of_c * 1.2, tg_of_c),
                               tg_of_c))

ggplot(treatment_ratios, aes(censusdate, tg_of_c_toy, color = oplottype)) +
  geom_line() +
  scale_color_viridis_d(end = .8) +
  theme(legend.position = "top")+ 
  facet_wrap(vars(era), scales= "free_x", ncol = 1)

tg_toy_ratio_gam <- gam(tg_of_c_toy ~ fplottype + s(period, k = 100) + s(period, by = fplottype, k =100), family = "Gamma", data = treatment_ratios)

tg_toy_ratio_pdat <- select(treatment_ratios, era, fplottype, period) %>%
  mutate(type = "tinygran")

tg_toy_ratio_pred <- get_predicted_vals(tg_toy_ratio_gam, tg_toy_ratio_pdat)

tg_toy_ratio_ee_cc_diff <- get_exclosure_diff(tg_toy_ratio_gam, tg_toy_ratio_pred, comparison_variable = "fplottype", comparison_level = 3)

tg_toy_ratio_ee_cc_pred <- add_exclosure_diff(tg_toy_ratio_pred, tg_toy_ratio_ee_cc_diff)

plot_fitted_pred(filter(tg_toy_ratio_ee_cc_pred, fplottype != "CE"), comparison_variable = "fplottype") +
  geom_hline(yintercept = 1) +
  ggtitle("GAM diff tg_toyran ratio EE v CC")+
  facet_grid(cols = vars(era), scales = "free_x", space = "free") +
  theme(legend.position = "top")


tg_toy_ratio_ce_cc_diff <- get_exclosure_diff(tg_toy_ratio_gam, tg_toy_ratio_pred, comparison_variable = "fplottype", comparison_level = 2)

tg_toy_ratio_ce_cc_pred <- add_exclosure_diff(tg_toy_ratio_pred, tg_toy_ratio_ce_cc_diff)

plot_fitted_pred(filter(tg_toy_ratio_ce_cc_pred, fplottype != "EE"), comparison_variable = "fplottype") +
  geom_hline(yintercept = 1)  +
  scale_fill_viridis_d(end = .4) +
  scale_color_viridis_d(end = .4) +
  ggtitle("GAM diff tg_toyran ratio CE v CC")+
  facet_grid(cols = vars(era), scales = "free_x", space = "free") +
  theme(legend.position = "top")


tg_toy_ratio_ce_ee_diff <- get_exclosure_diff(tg_toy_ratio_gam, tg_toy_ratio_pred, comparison_variable = "fplottype", comparison_level = 3, reference_level = 2)

tg_toy_ratio_ce_ee_pred <- add_exclosure_diff(tg_toy_ratio_pred, tg_toy_ratio_ce_ee_diff)

plot_fitted_pred(filter(tg_toy_ratio_ce_ee_pred, fplottype != "CC"), comparison_variable = "fplottype") +
  geom_hline(yintercept = 1) +
  scale_fill_viridis_d(begin = .4, end = .8) +
  scale_color_viridis_d(begin = .4, end = .8) +
  ggtitle("GAM diff tg_toyran ratio CE v EE")+
  facet_grid(cols = vars(era), scales = "free_x", space = "free") +
  theme(legend.position = "top")
```
