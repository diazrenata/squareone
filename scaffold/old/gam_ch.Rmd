...
load(here::here("scaffold", "Reports", "te_gams.RData"))
...
te_pred <- predict_treat_effect2(plot_totals, 500, te.gam, exVars = NULL)
te_pred2 <- predict_treat_effect2(plot_totals, 500, te.gam, exVars = exVars.d)

all_equal(te_pred, te_pred2)

ggplot(te_pred, aes(numericdate, Fitted, color = oTreatment, fill = oTreatment)) +
  geom_line() +
  geom_ribbon(aes(ymin= Lower, ymax = Upper), alpha= .5)
te_diff <- osmooth_diff(te.gam, te_pred, "numericdate", "CC", "EE", "oTreatment")
ggplot(te_diff, aes(censusdate, diff)) + geom_line() + geom_ribbon(aes(ymin = lower, ymax= upper), alpha = .5)


te.noplot.pred <- predict_treat_effect2(plot_totals, 500, te.gam.noplot, exVars = NULL)

ggplot(te.noplot.pred, aes(numericdate, Fitted, color = oTreatment, fill = oTreatment)) +
  geom_line() +
  geom_ribbon(aes(ymin= Lower, ymax = Upper), alpha= .5)
te_diff <- osmooth_diff(te.gam.noplot, te.noplot.pred, "numericdate", "CC", "EE", "oTreatment")
ggplot(te_diff, aes(censusdate, diff)) + geom_line() + geom_ribbon(aes(ymin = lower, ymax= upper), alpha = .5)


te.randomplot.pred <- predict_treat_effect2(plot_totals, 500, te.gam.randomplot, exVars = NULL)

ggplot(te.randomplot.pred, aes(numericdate, Fitted, color = oTreatment, fill = oTreatment)) +
  geom_line() +
  geom_ribbon(aes(ymin= Lower, ymax = Upper), alpha= .5)
te_diff <- osmooth_diff(te.gam.randomplot, te.randomplot.pred, "numericdate", "CC", "EE", "oTreatment")
ggplot(te_diff, aes(censusdate, diff)) + geom_line() + geom_ribbon(aes(ymin = lower, ymax= upper), alpha = .5)
