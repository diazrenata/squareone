...
ggplot(sg_ratio_pred, aes(period, invlink_fit, color = fplottype, fill = fplottype)) +
  ...
  geom_line(data = treatment_ratios, aes(x = period, y = sg_of_c, color = fplottype), inherit.aes= F) +
...
ggplot(tg_ratio_pred, aes(period, invlink_fit, color = fplottype, fill = fplottype)) +
  geom_line() +
  geom_ribbon(aes(ymin = invlink_lower, ymax = invlink_upper), alpha = .5) +
  geom_line(data = treatment_ratios, aes(x = period, y = tg_of_c, color = fplottype), inherit.aes= F) +
  facet_grid(cols = vars(era), rows = vars(fplottype), space = "free", scales = "free")
```
...
ggplot(total_e_ratio_pred, aes(period, invlink_fit, color = fplottype, fill = fplottype)) +
  geom_line() +
  geom_ribbon(aes(ymin = invlink_lower, ymax = invlink_upper), alpha = .5) +
  geom_line(data = treatment_ratios, aes(x = period, y = total_e_of_c, color = fplottype), inherit.aes= F) +
  facet_grid(cols = vars(era), rows = vars(fplottype), space = "free", scales = "free")
```
...
era_means <- era_means %>%
  mutate(treatment_era = ifelse(oplottype == "CE", ifelse(era == "d_post-switch", "exclosure", "control_e"), ifelse(oplottype == "EE", "exclosure", "control")))

select(era_means, era, treatment_era, total_e_mean, smgran_e_mean, tinygran_e_mean) %>%
  group_by(era, treatment_era) %>%
  summarize(mean_e = mean(total_e_mean))
  distinct()
```
