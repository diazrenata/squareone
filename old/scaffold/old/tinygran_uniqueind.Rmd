...
plot_totals <- get_unique_inds(use_christensen_plots = use_christensen_plots, return_plot = T) %>%
   mutate(tinygran_e = smgran_e - pb_e) %>%
  mutate(oplottype = ordered(plot_type))

treatment_means <- get_unique_inds(use_christensen_plots, F, F) %>%
  mutate(tinygran_e = smgran_e - pb_e) %>%
  mutate(oplottype = as.factor(plot_type))  %>%
  mutate(period = wateryear)
...
ggplot(treatment_means, aes(period, total_e, color = plot_type)) +
  geom_line() +
  scale_color_viridis_d(end = .8) +
  ggtitle("Total E wateryear totals by treatment")


ggplot(treatment_means, aes(period, smgran_e, color = plot_type)) +
  geom_line() +
  scale_color_viridis_d(end = .8) +
  ggtitle("Small granivore E wateryear totals by treatment")


ggplot(treatment_means, aes(period, tinygran_e, color = plot_type)) +
  geom_line() +
  scale_color_viridis_d(end = .8) +
  ggtitle("Tiny gran E wateryear totals by treatment")

```

```{r}
sg_e_mod <- gam(smgran_e ~ oplottype + s(period, k = 30) + s(period, by = oplottype, k = 30), family = "tw", data = treatment_means)
...
sg_ratio_gam <- gam(sg_of_c ~ fplottype + s(period, k = 30) + s(period, by = fplottype, k = 30), family = "Gamma", data = treatment_ratios)
...
ggplot(filter(treatment_ratios), aes(wateryear, sg_of_c, color = fplottype)) +
  geom_line() +
  scale_color_viridis_d(end = .8) +
  ggtitle("Sgran ratio by plot type") +
  geom_hline(yintercept = 1)
...
tg_ratio_gam <- gam(tg_of_c ~ fplottype + s(period, k = 30) + s(period, by = fplottype, k =30), family = "Gamma", data = treatment_ratios)
...
ggplot(filter(treatment_ratios), aes(wateryear, tg_of_c, color = fplottype)) +
  geom_line() +
  scale_color_viridis_d(end = .8) +
  ggtitle("Tgran ratio by plot type") +
  geom_hline(yintercept = 1)
```

```{r}

total_ratio_gam <- gam(total_e_of_c ~ fplottype + s(period, k = 20) + s(period, by = fplottype, k = 20), family = "Gamma", data = filter(treatment_ratios, fplottype != "CC"))
...
ggplot(filter(treatment_ratios), aes(wateryear, total_e_of_c, color = fplottype)) +
  geom_line() +
  scale_color_viridis_d(end = .8) +
  ggtitle("Total energy ratio by plot type") +
  geom_hline(yintercept = 1)
