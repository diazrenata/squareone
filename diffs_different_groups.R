...
#exVars.d = names(coefficients(te.gam))[ which(grepl("oPlot", names(coefficients(te.gam))))]
exVars.d <- c('oPlot', paste0('s(numericdate):oPlot', unique(plot_totals$oPlot)))
te.gam.pred<- predict_treat_effect2(plot_totals, 500, te.gam, exVars.d)

ggplot(te.gam.pred, aes(censusdate, Fitted, color = oTreatment, fill = oTreatment)) +
  geom_line() +
  geom_ribbon(aes(ymin = Lower, ymax = Upper), alpha = .5)

te.gam.fitted <- gratia::add_fitted(plot_totals, te.gam)

ggplot(te.gam.fitted, aes(censusdate, .value, color = oTreatment, group = oPlot)) +
  geom_line()
...
diff_preddat_te <- te.gam.pred %>%
  select(censusdate, oPlot, oTreatment, numericdate)

diffs_eria_te <- get_smooth_diffs(te.gam, diff_preddat)
